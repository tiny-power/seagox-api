 ...
 */
@Service
public class MinHandler implements AnnotationHandler {

	@Override
	public void afterPropertiesSet() throws Exception {
		AnnotationHandlerFactory.register("Min", this);
	}
	...
	public List<String> valid(String type, int row, int col, String cellValue, String annotation, List<String> failList) {
		...
		if (Integer.valueOf(cellValue) <= min) {
			failList.add("第" + (row + 1) + "行" + ExcelUtils.letterList.get(col) + "列错误：" + "数值不能小于" + min);
		}
		return failList;
	}
